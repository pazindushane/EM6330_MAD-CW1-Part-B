import 'package:flutter/material.dart';

class DarkColor {
  static const Color lightpurple = Color(0XFFC9B3EA);
  static const Color darkpurple = Color(0XFF83679A);
  static const Color grey = Color(0xff9D99A7);
  static const Color darkgrey = Color(0xff625f6a);

  static const Color brighter = Color(0xFFC54242);
  static const Color lightblack = Color(0xff3E404D);

  static const Color background = Color(0xff101010);
  static const Color black = Color(0xff000000);
  static const Color white = Color(0xffffffff);
  static const Color lightGrey = Color(0xff797878);
  static const Color darkBlue = Color(0xFFB30101);
  static const Color lightBlue = Color(0xFFC54242);
  static const Color darkPink = Color(0xFFB20000);
  static const Color purple = Color(0xFFC44040);

  static const Color Brighter = Color(0xff1a1a1c);
  static const Color Darker = Colors.black;
  static const Color titleTextColor = const Color(0xffd1d1d0);
  static const Color subTitleTextColor = const Color(0xff797878);
}
